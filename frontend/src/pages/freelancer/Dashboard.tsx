...
import { useJobs, Application, Job } from '@/hooks/useJobs';
...
const FreelancerDashboard = () => {
  ...
  useEffect(() => {
    if (user) {
      const freelancerApplications = getUserApplications();
  ...
  const getJobForApplication = (jobId: string): Job | undefined => {
                ...
                <div className="text-center">
                  <p className="text-4xl font-bold text-brand-blue">
                    {applications.filter(app => 
                      getJobForApplication(app.jobId)?.status === 'open'
                    ).length}
                  </p>
