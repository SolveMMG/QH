...
const JobDetail = () => {
  ...
  const { jobs, applications, applyToJob, archiveJob, updateJob } = useJobs();
  const { user } = useAuth();
  ...
  useEffect(() => {
    const fetchApplications = async () => {
      if (job && user?.role === 'employer' && user.id === job.employerId) {
        const apps = await fetchApplicationsForJob(job.id, user);
  ...
  const handleUpdateJobStatus = async (status: 'open' | 'closed' | 'archived') => {
    ...
    try {
      ...
      toast.success(`Job ${status === 'open' ? 'reopened' : 'closed'} successfully`);
    } catch (error) {
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 bg-gray-50 py-8">
        <div className="container mx-auto px-4">
          <div className="mb-6">
            <Link to="/jobs" className="text-brand-blue hover:underline flex items-center">
              ← Back to Jobs
            </Link>
          </div>
                        ...
                        <CardContent className="p-6">
                          <div className="text-center">
                            ...
                            <p className="text-gray-600">
                              You have already applied to this job. The employer will review your
                              application.
                            </p>
                          </div>
                        </CardContent>
                  ...
                  <Card>
                    <CardContent className="p-6">
                      ...
                      <ol className="list-decimal list-inside space-y-2 text-gray-700">
                        ...
                        <li>Ensure your skills match what the employer is looking for.</li>
